...
- (void)takePicture:(CDVInvokedUrlCommand*)command
{
    ...
    if (true) { //([self popoverSupported] && (sourceType != UIImagePickerControllerSourceTypeCamera)) {
...
- (void)displayPopover:(NSDictionary*)options
{
    NSInteger x = 100; //0;
    NSInteger y = 250; //32;
    NSInteger width = 160; //320;
    NSInteger height = 160; //480;
    UIPopoverArrowDirection arrowDirection = UIPopoverArrowDirectionAny;

    /*if (options) {
        x = [options integerValueForKey:@"x" defaultValue:0];
        y = [options integerValueForKey:@"y" defaultValue:32];
        width = 160; //[options integerValueForKey:@"width" defaultValue:320];
        height = 240; //[options integerValueForKey:@"height" defaultValue:480];
